<?php
	...
	$moduleCode = $_POST['module_code_select'];
	 ...
	 $room_code = $_POST['roomCode0'];
	
	$room_code2; if($n > 1)$room_code2=$_POST['roomCode1'];
	$room_code3; if($n > 2) $room_code3=$_POST['roomCode2'];
	$room_code4; if($n > 3) $room_code4=$_POST['roomCode3'];
    ...
    $bookingsql1 = "INSERT INTO BOOKING(request_id, room_code) VALUES (".$id1.", '".$room_code."')";
	if($n > 1) $bookingsql2 = "INSERT INTO BOOKING(request_id, room_code) VALUES (".$id2.", '".$room_code2."')";
	if($n > 2) $bookingsql3 = "INSERT INTO BOOKING(request_id, room_code) VALUES (".$id3.", '".$room_code3."')";
	if($n > 3) $bookingsql4 = "INSERT INTO BOOKING(request_id, room_code) VALUES (".$id4.", '".$room_code4."')";
	$res =& $db->query($bookingsql1); 
				if(PEAR::isError($res)){
					die($res->getMessage());
				} 
	...
	if($n > 1){
		$sql2 = 'INSERT INTO REQUEST(request_id, dept_code, module_code, room_code, capacity, wheelchair, projector, visualiser, whiteboard, special_Requirements, priority, period, day, duration, req_group)
           VALUES (' . ($id2) .',\''. $deptCode .'\',\''. $moduleCode .'\',\''. $room_code2 .'\',\''. $cap2 .'\','. $wheelchair2 .','. $projector2 .','. $visualiser2 .','. $whiteboard2 .',\''. $specialRequirements .'\','.  $priority .','. $period .',\''. $day .'\','. $duration .','. $id1 .');';
	   
		$res =& $db->query($sql2); 
		if(PEAR::isError($res)){
			die($res->getMessage());
		}
		$res =& $db->query($bookingsql2); 
		if(PEAR::isError($res)){
			die($res->getMessage());
		}
	...
	if($n > 2){
		$sql3 = 'INSERT INTO REQUEST(request_id, dept_code, module_code, room_code, capacity, wheelchair, projector, visualiser, whiteboard, special_Requirements, priority, period, day, duration, req_group)
           VALUES (' . ($id3) .',\''. $deptCode .'\',\''. $moduleCode .'\',\''. $room_code3 .'\',\''. $cap3 .'\','. $wheelchair3 .','. $projector3 .','. $visualiser3 .','. $whiteboard3 .',\''. $specialRequirements .'\','.  $priority .','. $period .',\''. $day .'\','. $duration .','. $id1 .');';
		
		$res =& $db->query($sql3); 
		if(PEAR::isError($res)){
			die($res->getMessage());
		}
		$res =& $db->query($bookingsql3); 
		if(PEAR::isError($res)){
			die($res->getMessage());
		}
	...
	if($n > 3){
		$sql4 = 'INSERT INTO REQUEST(request_id, dept_code, module_code, room_code, capacity, wheelchair, projector, visualiser, whiteboard, special_Requirements, priority, period, day, duration, req_group)
           VALUES (' . ($id4) .',\''. $deptCode .'\',\''. $moduleCode .'\',\''. $room_code4 .'\',\''. $cap4 .'\','. $wheelchair4 .','. $projector4 .','. $visualiser4 .','. $whiteboard4 .',\''. $specialRequirements .'\','.  $priority .','. $period .',\''. $day .'\','. $duration .','. $id1 .');';
		
		$res =& $db->query($sql4); 
		if(PEAR::isError($res)){
			die($res->getMessage());
		}
		$res =& $db->query($bookingsql4); 
		if(PEAR::isError($res)){
			die($res->getMessage());
		}
